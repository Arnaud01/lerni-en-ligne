(function(){   // anonymous wrapper : rien n'a besoin d'être global
	...
	var app = angular.module('myApp.controllerWriteWords',[]);


	app.controller('CtrlWriteWords', function($scope, $http, $localStorage) {
	...
	// _________________test si la variable prenomEcrireMots existe _____________________//
		...
		$scope.hear = function() {
			...
			if (audio.canPlayType('audio/ogg;')) {
				...
				source.src= 'sounds/mots/' + $scope.serie_aleatoire[$scope.n] + '.ogg';

			} else {
				source.type= 'audio/mpeg';
				source.src= 'sounds/mots/' + $scope.serie_aleatoire[$scope.n] + '.mp3';
			}
		...
		$scope.note = $scope.tauxReussite - $scope.tauxErreur ;
			...
			if ($scope.note == 10) { // si tout est juste : vert
				...
				if (typeof $scope.prenomEcrireMots != 'undefined') { // si la variable prenoms existe
					...
					$scope.prenomEcrireMots[$scope.serie] = 'green';

					$localStorage[$scope['prenomCompile']] = $scope.prenomEcrireMots; // écriture
					...
					$localStorage[$scope['prenomCompile']] = $scope.prenomEcrireMots; // mise à jour
			...
			if (typeof $scope.prenomEcrireMots != 'undefined') { // si la variable prenoms existe
				...
				$scope.prenomEcrireMots[$scope.serie] = 'yellow';
			...
			if (typeof $scope.prenomEcrireMots != 'undefined') { // si la variable prenoms existe
				...
				$scope.prenomEcrireMots[$scope.serie] = 'orange';
			...
			if (typeof $scope.prenomEcrireMots != 'undefined') { // si la variable prenoms existe
				...
				$scope.prenomEcrireMots[$scope.serie] = 'red';
